...
public class Bet {
    ...
    public long[] allCancel() {
        ...
        for (BetItem item : this.betItems) {
            ...
            item.setResult(WebUtils.role().getValue() == Role.USER.getValue() ? "베팅취소" : (item.isStart() ? "진행중취소" : "취소"));
    ...
    private void makeCancel() {
        this.cancel = true;
        this.result = WebUtils.role().getValue() == Role.USER.getValue() ? "베팅취소" : (this.isStart() ? "진행중취소" : "취소");
        this.hitMoney = this.betMoney;
        this.hitOdds = 1;
        this.closing = true;
    }
