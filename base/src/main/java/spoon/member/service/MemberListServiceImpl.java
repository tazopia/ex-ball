...
public class MemberListServiceImpl implements MemberListService {
    ...
    @Override
    public Page<Member> sellerList(MemberDto.Seller command, Pageable pageable) {
    ...
    private void builderBySearch(BooleanBuilder builder, MemberDto.Command command) {
        ...
        switch (command.getSearchType()) {
            case "agency":
                builder.and(q.agency1.like(command.getSearchValue() + "%").or(q.agency2.like(command.getSearchValue() + "%")));
                break;
            case "recommender":
    ...
    private void builderByMode(BooleanBuilder builder, String mode) {
        switch (mode) {
            ...
            case "dummy":
                ...
                break;
            case "agency":
                builder.and(q.role.in(Role.AGENCY1, Role.AGENCY2));
                break;
            case "admin":
                Role role = WebUtils.role();
                if (role == Role.GOD) {
            ...
            case "disabled":
                ...
                break;
            case "secession":
                builder.and(q.role.eq(Role.USER)).and(q.secession.eq(true));
                break;
            case "profile":
