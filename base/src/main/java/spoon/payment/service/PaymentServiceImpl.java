...
public class PaymentServiceImpl implements PaymentService {
    ...
    public AjaxResult exchange(PaymentDto.Add add, String bankPassword) {
        ...
        try {
            addPoint(PointCode.CHANGE, 0L, member.getUserid(), -add.getAmount(), "포인트 보냄 - " + add.getUserid() + (member.getRole() == Role.AGENCY2 ? " 대리점" : " 회원"));
            addPoint(PointCode.CHANGE, 0L, add.getUserid(), add.getAmount(), "포인트 받음 - " + member.getUserid() + (member.getRole() == Role.AGENCY2 ? " 총판" : " 대리점"));
        } catch (RuntimeException e) {
