...
public class KenoLadderGameServiceImpl implements KenoLadderGameService {
    ...
    public AjaxResult betting(ZoneDto.Bet betting) {
        ...
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.KENO_LADDER)).and(qi.groupId.eq(kenoLadder.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.cancel.isFalse()));
            if (count > 0) {
