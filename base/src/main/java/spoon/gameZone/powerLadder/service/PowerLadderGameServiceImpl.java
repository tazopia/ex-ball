...
@AllArgsConstructor
@Service
public class PowerLadderGameServiceImpl implements PowerLadderGameService {
    ...
    public AjaxResult betting(ZoneDto.Bet betting) {
        ...
        PowerLadder powerLadder = powerLadderRepository.findOne(q.sdate.eq(betting.getSdate()));
        Zone zone = powerLadder.getZone(betting.getGameCode());
        double odds = powerLadder.getOdds()[betting.getBetZone()];
        ...
        if (odds != betting.getOdds()) {
            result.setMessage("파워사다리 배당이 변경되었습니다. 페이지를 리로딩 하여 주세요.");
            return result;
        }

        // 배당이 정확한지
        if (odds < 1) {
            result.setMessage("파워사다리 베팅에 실패하였습니다. 다시 베팅하여 주세요.");
            return result;
        }

        // 베팅시간 체크
        if (powerLadder.getGameDate().getTime() - config.getBetTime() * 1000 < System.currentTimeMillis()) {
        ...
        long count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.POWER_LADDER)).and(qi.groupId.eq(powerLadder.getSdate())).and(qi.cancel.isFalse()));
        if (count >= config.getBetMax()) {
            result.setMessage(String.format("파워사다리는 회차당 %d 회만 베팅 가능 합니다.", config.getBetMax()));
            return result;
        }

        // 동일조합 체크
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.POWER_LADDER)).and(qi.groupId.eq(powerLadder.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.cancel.isFalse()));
            if (count > 0) {
                result.setMessage("파워사다리는 동일 게임을 중복 베팅할 수 없습니다.");
                return result;
            }
        }
        ...
        bet.setMenuCode(MenuCode.POWER_LADDER);
        bet.setBetCount(1);
        bet.setBetDate(new Date());
        bet.setBetOdds(odds);
        bet.setStartDate(powerLadder.getGameDate());
        bet.setEndDate(powerLadder.getGameDate());
        ...
        if (success) {
            ...
            if (bet.getRole() == Role.USER) {
                addAmount(powerLadder.getSdate());
            }
        } else {
    ...
    @Transactional
    @Override
    public void rollbackPayment(PowerLadder powerLadder) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.POWER_LADDER).and(qi.groupId.eq(powerLadder.getSdate())).and(qi.cancel.isFalse()));
        betClosingService.rollbackBetting(betItems);
    }

    @Transactional
    @Override
    public void closingBetting(PowerLadder powerLadder) {
        QBetItem qi = QBetItem.betItem;
        Iterable<BetItem> betItems = betItemRepository.findAll(qi.menuCode.eq(MenuCode.POWER_LADDER).and(qi.groupId.eq(powerLadder.getSdate())).and(qi.cancel.isFalse()));
        for (BetItem item : betItems) {
            ...
            ZoneScore zoneScore = powerLadder.getGameResult(item.getSpecial());
    ...
    protected void addAmount(String sdate) {
        try {
            ...
            List<BetDto.ZoneAmount> list = betMapper.zoneAmount(MenuCode.POWER_LADDER, sdate);
