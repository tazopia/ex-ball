...
public class PowerGameServiceImpl implements PowerGameService {
    ...
    public AjaxResult betting(ZoneDto.Bet betting) {
        ...
        if (config.getBetMax() > 1) {
            count = betItemRepository.count(qi.userid.eq(userid).and(qi.menuCode.eq(MenuCode.POWER)).and(qi.groupId.eq(power.getSdate())).and(qi.special.eq(betting.getGameCode())).and(qi.cancel.isFalse()));
            if (count > 0) {
