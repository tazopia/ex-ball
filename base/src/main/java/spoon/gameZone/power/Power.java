...
public class Power {
    ...
    // 스코어 입력
    public void updateScore(PowerDto.Score score) {
        if (score.isCancel()) {
            this.pb = "";
            this.ball = "";
            this.oddeven = "";
    ...
    public Zone getZone(String gameCode) {
        ...
        switch (gameCode) {
            ...
            case "pb_even_ou":
                return getPowerZone("even_ou", "파워볼", "짝 [오버]", "짝 [언더]", 23, 24);
            default:
                throw new GameZoneException("파워볼 코드 " + gameCode + " 를 확인 할 수 없습니다.");
        }
    }
    ...
    private Zone getPowerZone(String gameCode, String type, String teamHome, String teamAway, int idxHome, int idxAway) {
        ...
        zone.setMenuCode(MenuCode.POWER);
        zone.setGameCode(gameCode);
        zone.setLeague(String.format("%03d회차 파워볼", this.round));
        ...
        zone.setHandicap(0);
        zone.setOddsHome(ZoneConfig.getPower().getOdds()[idxHome]);
        zone.setOddsDraw(0);
        zone.setOddsAway(ZoneConfig.getPower().getOdds()[idxAway]);
        zone.setGameDate(this.gameDate);
        return zone;
    }

    public boolean isChangeResult(PowerDto.Score score) {
